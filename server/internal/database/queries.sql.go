...
RETURNING id, name, icon, description, fields, creator_id, created_at, deleted_at, is_public, fields_search, gdp_measure_field
`
...
type CreateObjectTypeParams struct {
	...
	GdpMeasureField sql.NullString  `json:"gdp_measure_field"`
}
...
const getAccessibleObjectTypesForMember = `-- name: GetAccessibleObjectTypesForMember :many
SELECT obj_type_id
FROM creator_obj_type_access
WHERE creator_id = $1
`

func (q *Queries) GetAccessibleObjectTypesForMember(ctx context.Context, creatorID uuid.UUID) ([]uuid.UUID, error) {
	rows, err := q.query(ctx, q.getAccessibleObjectTypesForMemberStmt, getAccessibleObjectTypesForMember, creatorID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []uuid.UUID
	for rows.Next() {
		var obj_type_id uuid.UUID
		if err := rows.Scan(&obj_type_id); err != nil {
			return nil, err
		}
...
SELECT id, name, icon, description, fields, creator_id, created_at, deleted_at, is_public, fields_search, gdp_measure_field FROM obj_type
WHERE id = $1 AND deleted_at IS NULL
LIMIT 1
`
...
const grantAccessToObjectType = `-- name: GrantAccessToObjectType :exec
INSERT INTO creator_obj_type_access (creator_id, obj_type_id)
VALUES ($1, $2)
ON CONFLICT DO NOTHING
`

type GrantAccessToObjectTypeParams struct {
	CreatorID uuid.UUID `json:"creator_id"`
	ObjTypeID uuid.UUID `json:"obj_type_id"`
}

func (q *Queries) GrantAccessToObjectType(ctx context.Context, arg GrantAccessToObjectTypeParams) error {
	_, err := q.exec(ctx, q.grantAccessToObjectTypeStmt, grantAccessToObjectType, arg.CreatorID, arg.ObjTypeID)
	return err
}
...
func (q *Queries) ListAccessibleObjectTypes(ctx context.Context, arg ListAccessibleObjectTypesParams) ([]ListAccessibleObjectTypesRow, error) {
	...
	defer rows.Close()
	var items []ListAccessibleObjectTypesRow
	for rows.Next() {
		var i ListAccessibleObjectTypesRow
		if err := rows.Scan(
			&i.ID,
			&i.Name,
			&i.Icon,
...
const revokeAccessToObjectType = `-- name: RevokeAccessToObjectType :exec
DELETE FROM creator_obj_type_access
WHERE creator_id = $1 AND obj_type_id = $2
`

type RevokeAccessToObjectTypeParams struct {
	CreatorID uuid.UUID `json:"creator_id"`
	ObjTypeID uuid.UUID `json:"obj_type_id"`
}

func (q *Queries) RevokeAccessToObjectType(ctx context.Context, arg RevokeAccessToObjectTypeParams) error {
	_, err := q.exec(ctx, q.revokeAccessToObjectTypeStmt, revokeAccessToObjectType, arg.CreatorID, arg.ObjTypeID)
	return err
}
