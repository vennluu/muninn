...
const mergeObjects = `-- name: MergeObjects :exec
WITH target_org AS (
...
type MergeObjectsParams struct {
...
func (q *Queries) MergeObjects(ctx context.Context, arg MergeObjectsParams) error {
	_, err := q.exec(ctx, q.mergeObjectsStmt, mergeObjects, arg.TargetObjectID, pq.Array(arg.SourceObjectIds), arg.CreatorID)
	return err
}

const validateMergeObjects = `-- name: ValidateMergeObjects :one
WITH obj_check AS (
...
type ValidateMergeObjectsParams struct {
	Column1 []uuid.UUID `json:"column_1"`
	ID      uuid.UUID   `json:"id"`
}

type ValidateMergeObjectsRow struct {
	ValidationResult string      `json:"validation_result"`
	CreatorOrgID     uuid.UUID   `json:"creator_org_id"`
	ObjectsOrgID     interface{} `json:"objects_org_id"`
}

func (q *Queries) ValidateMergeObjects(ctx context.Context, arg ValidateMergeObjectsParams) (ValidateMergeObjectsRow, error) {
	row := q.queryRow(ctx, q.validateMergeObjectsStmt, validateMergeObjects, pq.Array(arg.Column1), arg.ID)
	var i ValidateMergeObjectsRow
	err := row.Scan(&i.ValidationResult, &i.CreatorOrgID, &i.ObjectsOrgID)
	return i, err
}
