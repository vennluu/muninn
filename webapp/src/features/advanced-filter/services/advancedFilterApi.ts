...
export const fetchAdvancedFilterResults = async (
  ...
  if (filterConfig.tagIds?.length)
    params.tag_ids = filterConfig.tagIds.join(",");
  if (filterConfig.typeIds?.length)
    params.type_ids = filterConfig.typeIds.join(",");
  ...
  if (filterConfig.funnelStepFilter?.funnelId) {
    if (filterConfig.funnelStepFilter.stepIds?.length) {
      params.step_ids = filterConfig.funnelStepFilter.stepIds.join(",");
    }

    if (filterConfig.funnelStepFilter.subStatuses?.length) {
